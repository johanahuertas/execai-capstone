...
def search_contacts_service(provider: str, query: str, max_scan: int = 50) -> List[Dict[str, str]]:
    ...
    try:
        emails_data = list_emails_service(
            provider=provider,
    ...
    own_email = max(to_counts, key=to_counts.get) if to_counts else ""
    ...
    if query:
        contacts.sort(
            key=lambda c: (
                0 if c["name"].lower().startswith(query) else 1,
                c["name"].lower(),
            )
        )
    else:
