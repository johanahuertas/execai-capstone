...
def prov_name(p=None):
    p = p or prov()
    return "Outlook" if p == "outlook" else "Google"
    ...
    try:
        res = requests.post(
            ...
            json={
                "text": prompt,
                "provider": prov(),
                "last_context": _build_last_context(),
            },
            timeout=25,
        )
...
def book(title, start, duration_min, attendees=None, pending_reply=None, pending_draft=None):
    ...
    try:
        res = requests.post(
            f"{API_BASE}/integrations/{p}/create-event",
            json={
                "title": title,
                "start": start,
        ...
        if pending_reply:
            try:
                ...
                if start:
                    try:
                        from zoneinfo import ZoneInfo
                        ts = datetime.fromisoformat(start).astimezone(
                            ZoneInfo("America/New_York")
                ...
                r2 = requests.post(
                    f"{API_BASE}/integrations/{p}/create-reply-draft",
                    json={
        ...
        if pending_draft:
            try:
                ...
                r3 = requests.post(
                    f"{API_BASE}/integrations/{p}/create-draft",
                    json={
        ...
        push({"action": "create_event"}, {"status": "error", "detail": str(e)})
        st.rerun()
...
def event_card_with_delete(event: dict, provider: str):
    ...
    with st.container(border=True):
        ...
        if event_id and st.button("🗑 Delete", key=f"delbtn_{event_id}", type="secondary"):
            try:
                requests.delete(
                    f"{API_BASE}/integrations/{provider}/events/{event_id}",
                    timeout=15,
                ).raise_for_status()
...
def time_option_card(
    ...
    with st.container(border=True):
        ...
        if attendees:
            st.caption("👥 " + "  ·  ".join(attendees))
        if pending_reply:
            st.caption("_Will also create the reply draft._")
        if pending_draft:
            st.caption("_Will also create the email draft._")
...
def render_read_email(result: dict):
    ...
    with st.container(border=True):
        ...
        st.text_area(
            "Body",
            value=clean_body(em.get("body", "")),
            height=260,
            disabled=True,
            key=f"body_{em.get('id', 'x')}",
        )
...
def render_meeting_options(decision: dict, result: dict, key_prefix: str = "sug"):
    ...
    if not options:
        st.info(result.get("message") or "No options found.")
        return

    st.markdown("##### 🗓 Suggested times")
    if busy_disp:
        st.caption("Already busy: " + "  ·  ".join(busy_disp))
...
def render_generic(decision: dict):
    ...
    elif msg in {"Here's the updated draft.", "Here's the updated reply."}:
        return
    else:
        st.info(msg)
...
def render_reply_and_create(result: dict):
    ...
    if s == "success":
        ...
        if result.get("reply"):
            render_reply_draft(result["reply"])
    ...
    if s == "partial_success":
        ...
        if cs == "conflict_detected":
            ...
            render_alternatives(
                cal.get("alternatives") or [],
                cal.get("proposed_event") or {},
                key_prefix="r_alt",
                pending_reply=pending_reply,
            )
        elif cs == "needs_clarification":
            render_needs_clarification(cal)
        elif cs == "created":
            render_created_event(cal)
        else:
...
def render_draft_and_create(result: dict):
    ...
    if s == "success":
        st.success(result.get("message", "Done."))
        if result.get("draft"):
            render_created_draft(result["draft"])
        if (result.get("calendar") or {}).get("status") == "created":
            render_created_event(result["calendar"])
        return
...
def render_result(decision: dict, result, idx: int = 0):
    ...
    if action == "read_email" and "email" in (result or {}):
        render_read_email(result)
        return

    if action in {"create_draft", "draft_email", "revise_draft"}:
    ...
    if action in {"reply_email", "revise_reply_draft"}:
        s = (result or {}).get("status")
        if s == "reply_draft_created":
            render_reply_draft(result)
            return

    if action == "reply_and_create_event":
        render_reply_and_create(result)
        return

    if action == "draft_email_and_create_event":
        render_draft_and_create(result)
        return

    if action == "suggest_times":
        render_meeting_options(
            decision,
            result,
            key_prefix=f"s_{idx}_{abs(hash(str(decision)+str(result)))}",
        )
        return
...
with st.sidebar:
    ...
    if not st.session_state.google_status:
        if st.button("Connect Google", use_container_width=True):
            try:
                res = requests.get(
                    f"{API_BASE}/integrations/google/auth-url",
                    timeout=15,
    ...
    if not st.session_state.outlook_status:
        if st.button("Connect Outlook", use_container_width=True):
            try:
                res = requests.get(
                    f"{API_BASE}/integrations/outlook/auth-url",
                    timeout=15,
                    allow_redirects=False,
                )
                url = res.headers.get("Location") if 300 <= res.status_code < 400 else res.json().get("auth_url")
...
with st.expander("🛠 Quick tools", expanded=False):
    ...
    with tc1:
        ...
        if st.button("Load next 7 days", use_container_width=True, key="t_meet"):
            try:
                data = requests.post(
                    f"{API_BASE}/integrations/{prov()}/list-events",
                    json={"days": 7},
                    timeout=20,
                ).json()
    ...
    with tc2:
        ...
        if st.button("Check free time", use_container_width=True, key="t_free"):
            try:
                ...
                data = requests.post(
                    f"{API_BASE}/integrations/{prov()}/freebusy",
                    json={
                        "time_min": s.isoformat(),
                        "time_max": e.isoformat(),
                ...
                if busy:
                    for b in busy:
                        st.caption(f"Busy: {fmt(b['start'])} → {fmt(b['end'])}")
                else:
    ...
    with tc3:
        ...
        if st.button("Create draft", use_container_width=True, key="t_draft"):
            try:
                requests.post(
                    f"{API_BASE}/integrations/{prov()}/create-draft",
